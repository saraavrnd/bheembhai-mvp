...
- **Two signals on separate channels**: the result payload (from container, at `/out/bb_step_result.json`) and the exit status (polled from the runtime via `Runtime.status()`). A reconciler joins them.
...
- **Workdir path parity (non-negotiable)**: the engine passes `BB_WORKDIR` paths to the docker daemon, which resolves them on the **host** — so the workdir must exist at the same absolute path on the host and inside the engine container (compose binds `${PWD}/bb-workdir` at that path and sets `BB_WORKDIR` to it). A container-only path (e.g. `/app/bb-workdir` under the `./:/app` mount) makes the daemon silently create a root-owned `0755` tree that the `node` agent can't write — git clone dies with "could not create work tree dir" and the step fails `failed_incomplete`

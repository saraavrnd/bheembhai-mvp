...
async def register_logs_at_launch(session, run, step_id: str, attempt_no: int,
                                  store) -> int:
    """Register the attempt's live-log reference rows AT LAUNCH, before any
    artifact exists in storage.

    ``agent.log`` is heartbeated to its final key every ~5 s while the
    container runs and ``container.log`` is captured by the engine at exit —
    but neither object exists yet when the container starts, and the logs
    endpoint can only serve a key it has a reference row for. These rows make
    the in-flight attempt tail-able: they are inserted optimistically with
    size 0, and ``upload_step_logs`` (termination) finalizes the real size.
    A size-0 row doubles as the "waiting for first upload" marker the platform
    renders as pending instead of a 404.

    Idempotent: an existing row for this attempt+kind is reset to 0 (its
    object was just cleared by _clear_attempt_channels on re-launch). Commits
    with the launch transition in the caller's transaction. Never raises."""
    if store is None:
        return 0
    added = 0
    for kind in ("agent", "container"):
        ...
        try:
            ...
            if existing is None:
                session.add(RunLog(
                    ...
                    kind=kind, object_key=key, size_bytes=0))
            else:
                existing.size_bytes = 0
            added += 1
        except Exception:  # noqa: BLE001 — best-effort bookkeeping must not fail the run
            logger.warning(
                "run_logs launch registration failed run=%s step=%s attempt=%s kind=%s:\n%s",
...
async def upload_step_logs(session, run, step_id: str, attempt_no: int,
    ...
    leave an artifact without a pointer. An existing row (e.g. the optimistic
    size-0 row registered at launch) gets its size finalized to the head size
    — the same treatment heals a stale size after a re-run of the attempt.
    Returns the number of rows added or confirmed (a row is never duplicated).
    Never raises."""
    if store is None:
        return 0
    added = 0
    for kind in KINDS:
        ...
        try:
            ...
            elif existing.size_bytes != head.size:
                existing.size_bytes = head.size
            added += 1

...
Layout** table inside `CLAUDE.md` at the repo root.

## The rule
1. Before reading or writing any project artifact, open `CLAUDE.md` (repo root) and read its
...
3. If `CLAUDE.md` or the Project Layout table is missing (e.g. very first run before
   `project-scaffold`), fall back to the default layout below and create artifacts there.

## Default layout (used only until CLAUDE.md exists)
...
stay inside that skill's folder (wherever the agent loads skills from, e.g. `.claude/skills/` or

...
| Step | Skill | Result |
|------|-------|--------|
| 0 | (story-implement start) | LNPRTL precondition checks passed; Jira LEARN-21 → **In Progress** |
...
`material` table in data-model.md, updated CLAUDE.md (endpoint + entity). Flagged **LEARN-22**
...
- Token cost (customfield_10105): $0.26 — per-model: opus (code-review/story-design) $0.11,
  sonnet (implement) $0.14, haiku (orchestration) $0.01; breakdown posted as a Jira comment.
  Source: JSONL (relative metric — opus thinking tokens undercounted; labeled on the comment).

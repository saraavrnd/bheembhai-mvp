...
name: design-sync
description: >
  Keep the system design references and CLAUDE.md continuously in sync with shipped code: when a
  story is ready to merge, reconcile what it changed against architecture.md, data-model.md,
  api-contracts/, and CLAUDE.md, commit the doc updates onto the SAME branch so they ride with
  ...
  just says "sync the design", "update the architecture docs", "update CLAUDE.md", "reconcile
...
compatibility: >
  ...
  tech-design, and CLAUDE.md. Commits doc updates to the story branch via git/GitHub MCP. Uses
...
# design-sync — Keep the Design & CLAUDE.md Current (per merge)
...
`architecture.md`, `data-model.md`, and `CLAUDE.md` to decide their "how". If those references
...
- Updated reference files (only those that actually changed), committed to the story branch:
  potentially `architecture.md`, `data-model.md`, `api-contracts/`, `CLAUDE.md`, and a running
...
### Step 3 — Update CLAUDE.md (first-class output)
`CLAUDE.md` is what every future story's agent reads first, so keep it fresh. Update, as needed:
...
- `templates/CLAUDE.md.tmpl` — the structure of the living CLAUDE.md this keeps current.

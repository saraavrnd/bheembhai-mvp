...
module description, the contract file, and the relevant CLAUDE.md summary lines. Touch only what
changed.
...
## Keep CLAUDE.md current but lean
CLAUDE.md is read first by every future story's agent. Update its summaries (modules, endpoints,
entities, conventions) but keep it an orientation file, not a spec — push detail to
architecture.md / data-model.md / contracts and link to them. Stale CLAUDE.md degrades every

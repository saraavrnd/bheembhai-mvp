...
9. **`CLAUDE.md` at the repo root** — the living orientation file, including the **Project Layout**
...
Project Layout table in `CLAUDE.md`:
```
docs/
...
own package at build time — don't hand-maintain a second copy. Keep `CLAUDE.md` at the repo root
...
- Configure test discovery to recurse sub-folders (pytest does by default; ensure no flat-only
  config). Document the convention in `testing-strategy.md` and `CLAUDE.md`.
...
### Step 6 — CI, hygiene, CLAUDE.md, and first commit
...
Copy the ADRs from `tech-design` into `docs/adr/`. Create **`CLAUDE.md` at the repo root** from
the design-sync `CLAUDE.md` template, filling in the stack and module map and — critically — the
